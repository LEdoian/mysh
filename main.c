...
static char *gen_prompt(void) {
	...
	if (path_max == -1) path_max = 1024;	// Hopefully reasonable upper bound
	char path[path_max];
...
static int repl(void) {
	...
	while (!exit) {
		...
		if (line == NULL) {
			...
			if (status == GLR_SIGNAL) continue;	// This is only SIGINT, other signals kill us by default and we do not catch them
			if (status == GLR_EOF) break;
			if (status == GLR_ERROR) errx(3, "gl_read_line: %s", gl_error_message(gl, NULL, 0));
		}
		if (strchr(line, '\0') == NULL) {
			// The line was too long
			// Warn the user and ask again
			// We don't care that the user cannot do what they want
			printf("WARNING: The line was too long, so nothing happened.\n");
			continue;
		}
		free(prompt);
